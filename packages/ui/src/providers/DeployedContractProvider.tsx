...
import { CrowdFundingAPI, type DeployedCrowdFundingAPI } from "@crowd-funding/crowd-funding-api";
...
export interface DeploymentProvider {
  ...
  readonly crowdFundingApi: DeployedCrowdFundingAPI | undefined;
...
const DeployedContractProvider = ({ 
  ...
  const [crowdFundingApi, setCrowdFundingApi] = useState<DeployedCrowdFundingAPI | undefined>(undefined);
  ...
  const onJoinContract = useCallback(async () => {
    ...
    try {
      const deployedAPI = await CrowdFundingAPI.joinCrowdFundingContract(
        walletContext.walletState.providers,
        contractAddress,
        logger
      );
      
      setCrowdFundingApi(deployedAPI);
      setHasJoined(true);
