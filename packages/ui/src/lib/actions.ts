...
import {
  CrowdFundingAPI,
  type CrowdFundingContractProviders,
  type DeployedCrowdFundingAPI,
...
): Promise<DeployedCrowdFundingAPI> => {
  let api;
  if (contractAddress) {
    ...
    api = await CrowdFundingAPI.deployCrowdFundingContract(providers, logger);
  }
